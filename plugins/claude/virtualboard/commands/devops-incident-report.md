...
description: >-
  Execute the VirtualBoard Generate Incident Report (GIR) workflow (devops.incident-report / DEVOPS-INCIDENT). Use when the user requests this named workflow.
---

<!-- Generated from prompts/agents/devops/DevOps-Generate_Incident_Report.md by tools/sync_claude_plugin.py. -->
...
- Create report at `$VB_ROOT/reports/deployments/incidents/IR-{YYYY-MM-DD}-{incident-id}.md`
...
- Ensure `$VB_ROOT/reports/deployments/incidents/` exists
...
Template source: `$VB_ROOT/templates/reports/html/devops-incident-report.html`.
...
2. Read `$VB_ROOT/templates/reports/README.md`, then print the authoritative
   ...
   python3 "$VB_ROOT/tools/render_report.py" \
     --template devops-incident-report \
     --describe
   ```

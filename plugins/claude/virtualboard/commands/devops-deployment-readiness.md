...
description: >-
  Execute the VirtualBoard Generate Deployment Readiness Report (GDRR) workflow (devops.deployment-readiness / DEVOPS-READINESS). Use when the user requests this named workflow.
---

<!-- Generated from prompts/agents/devops/DevOps-Generate_Deployment_Readiness_Report.md by tools/sync_claude_plugin.py. -->
...
- ID: `devops.deployment-readiness`
- Alias: `DEVOPS-READINESS`
...
- Create report at `$VB_ROOT/reports/deployments/readiness/DRR-{YYYY-MM-DD}-{environment}.md`
...
- Ensure `$VB_ROOT/reports/deployments/readiness/` exists
...
Template source: `$VB_ROOT/templates/reports/html/devops-deployment-readiness.html`.
...
2. Read `$VB_ROOT/templates/reports/README.md`, then print the authoritative
   ...
   python3 "$VB_ROOT/tools/render_report.py" \
     --template devops-deployment-readiness \
     --describe
   ```

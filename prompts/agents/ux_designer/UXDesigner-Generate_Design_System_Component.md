...
1. Load `templates/reports/html/ux-design-system.html`. The comment block at the top of
...
6. Substitute the per-template scalar placeholders:
   `COMPONENT_NAME`, `VERSION`, `STATUS`, `OVERVIEW_HTML`, `USAGE_HTML`, `ANATOMY_HTML`, `ACCESSIBILITY_NOTES_HTML`, `BEHAVIOR_HTML`.
7. Expand each `{#NAME}…{/NAME}` list block once per item using the
   per-template list placeholders: `HERO_META_CELLS`, `TOKENS`, `VARIANTS`, `PROPS`, `DO_LIST`, `DONT_LIST`. The per-item field names are
...
9. Write the rendered HTML next to the Markdown:
   `.virtualboard/design/systems/DS-{project}.html`.
...
`templates/reports/examples/ux-design-system.example.html`.

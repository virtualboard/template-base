...
1. Load `templates/reports/html/devops-deployment-readiness.html`. The comment block at the top of
...
6. Substitute the per-template scalar placeholders:
   `ENVIRONMENT`, `TARGET_VERSION`, `GO_NOGO`, `GO_NOGO_CLASS`, `EXECUTIVE_SUMMARY_HTML`, `KPI_CHECKS_TOTAL`, `KPI_CHECKS_PASSED`, `KPI_CHECKS_FAILED`, `KPI_CHECKS_BLOCKING`, `ROLLBACK_PLAN_HTML`, `KNOWN_RISKS_HTML`, `POST_DEPLOY_VERIFY_HTML`, `NEXT_DEPLOY_DATE`.
7. Expand each `{#NAME}…{/NAME}` list block once per item using the
   per-template list placeholders: `HERO_META_CELLS`, `READINESS_CHECKS`, `SIGNOFFS`, `APPROVED_DEPLOYERS`, `COMMUNICATION_PLAN`. The per-item field names are
...
9. Write the rendered HTML next to the Markdown:
   `.virtualboard/deployments/readiness/DRR-{YYYY-MM-DD}-{env}.html`.
...
`templates/reports/examples/devops-deployment-readiness.example.html`.
